...
const SendOtp = props => { 
    ...
    const otpHandler1 = text => {
        setOtp1(text);
        focusNextInput(0);
    };
    const otpHandler2 = text => {
        setOtp2(text);
        focusNextInput(1);
    };
    const otpHandler3 = text => {
        setOtp3(text);
        focusNextInput(2);
    };
    const otpHandler4 = text => {
        setOtp4(text);
    };

    const email = props.navigation.getParam('email')
    const forgotPasswordHandler = async () => {
        const otpValidate = `${otp1}${otp2}${otp3}${otp4}`;
        if (otpValidate != '1234') {
            return Alert.alert('Error', 'OTP Not Valid', [{ text: 'Ok' }]);
        }
        ...
        <ScrollView style={{ marginTop: Platform.OS == 'ios' ? 0 : 0, backgroundColor: '#F6F0E4' }}>
            ...
            <View style={styles.gradient}>
                <Card style={styles.authContainer}>
                    <Image style={{ height: 160, width: '100%', resizeMode: 'contain', marginBottom: -5 }}  source={require('../assets/new/otp.png')} />              
                    ...
                    <View>
                        <View style={styles.formControl}>
                            <View style={{
                                ...
                                {inputRefs.map((inputRef, index) => (
                                    <View key={index} style={{ marginLeft: index > 0 ? 10 : 0, width: '23%', backgroundColor: 'white', borderWidth: 1, borderColor: '#8391A1', opacity: .6, borderRadius: 10, padding: 10 }}>
                                        <TextInput
                                            ref={inputRef}
                                            onChangeText={(text) => {
                                                switch (index) {
                        ...
                        <View style={styles.buttonContainer}>
                            <TouchableOpacity onPress={forgotPasswordHandler} style={{ backgroundColor: '#7F7F7F', padding: 10, borderWidth: 2, borderColor: 'white', borderRadius: 10, width: '100%', alignSelf: 'center' }}>
                                <View>
                                    <Text style={{ color: 'white', textAlign: 'center', fontFamily: 'poppins-regular' }}>Verify</Text>
                                </View>
                            </TouchableOpacity>
                        </View>
                    </View>
