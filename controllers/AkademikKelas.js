...
const AkademikKelas = props => {
    ...
    let kelasData = [];
    const tmpKelasData = useSelector(state => state.akademikv2.kelasData);
    ...
    if (tmpKelasData.length > 0) {
        for (let i = 0; i < tmpKelasData.length; i++) {
            const noItem = parseInt(i) + 1;
            const item = tmpKelasData[i];
            kelasData.push(
                [
                    noItem,
                    item.tingkat,
    ...
    const tableHead = ['No', 'Tingkat', 'Daftar Kelas', 'Status'];
    let tableData = [];
    ...
    if (tableData.length > 0) {
        return (
            <View style={styles.container}>
                {/* <KeteranganHeader text1="Daftar Kelas / Rombongan Belajar Pada Unit Sekolah" text2="" /> */}
...
AkademikKelas.navigationOptions = navigationData => {
	return {
        headerTitle: 'Informasi Kelas',
