...
const AdministrasiGuru = props => {
    ...
    const kbmData = useSelector(state => state.kegiatan_belajar.kbm);
    ...
    useEffect(() => {
        ...
        dispatch(mKegiatanBelajar.fetchKBMGuruDefault());
    ...
    const presensiHandler = async () => {
        if (pencarianGuru != 'pilihGuru') {
            ...
            try {
                await dispatch(mKegiatanBelajar.fetchKBMGuru(selIdGuru));
    ...
    const tableHead = ['No', 'Data Mata Pelajaran KBM', 'Tingkat/Kelas', 'Lihat Detail'];
    let tableData = [];
    if (kbmData.length > 0) {
    ...
    const widthArr = [40, 160, 140, 80];
    const element = (data, index) => (
        <TouchableOpacity onPress={() => 
            props.navigation.navigate('AdministrasiGuruDetail', {
        ...
        >   
            <Text style={{ textAlign: 'center' }}><Ionicons name='ios-search' size={18} color={core.primaryColor} /></Text>
        </TouchableOpacity>
    );
                                        ...
                                        <TableWrapper key={index} style={{ flexDirection: 'row' }}>
                                            {
                                                rowData.map((cellData, cellIndex) => (
                                                    <Cell style={{ width: widthArr[cellIndex] }} key={cellIndex} data={cellIndex === 3 ? element(cellData, index) : cellData}  textStyle={styles.text}/>
                                                ))
                                            }
                                        </TableWrapper>
                                    ))
...
AdministrasiGuru.navigationOptions = navigationData => {
	return {
        headerTitle: 'Data Administrasi Guru',
...
export default AdministrasiGuru;