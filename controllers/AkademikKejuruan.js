...
const AkademikKejuruan = props => {
    ...
    let kejuruanData = [];
    const tmpKejuruanData = useSelector(state => state.akademikv2.kejuruanData);
    ...
    useEffect(() => {
        dispatch(mAkademikV2.fetchKejuruan(idSekolah));
    ...
    if (tmpKejuruanData.length > 0) {
        for (let i = 0; i < tmpKejuruanData.length; i++) {
            const noItem = parseInt(i) + 1;
            const item = tmpKejuruanData[i];
            kejuruanData.push(
                [
                    noItem,
                    item.kode,
    ...
    const tableHead = ['No', 'Kode Kejuruan', 'Daftar Kejuruan', 'Status'];
    let tableData = [];
    if (kejuruanData.length > 0) {
        tableData = kejuruanData;
    }

    if (tableData.length > 0) {
        return (
            <View style={styles.container}>
                {/* <KeteranganHeader text1="Daftar Kejuruan / Jenjang Pendidikan Pada Unit Sekolah" text2="" /> */}
...
AkademikKejuruan.navigationOptions = navigationData => {
	return {
        headerTitle: 'Informasi Kejuruan',
...
export default AkademikKejuruan;